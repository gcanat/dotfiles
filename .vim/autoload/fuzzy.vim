...
const MAX_ELEMENTS: number = 60000

export def KeyMaps()
  var keymaps = execute("map")->split("\n")
  popup.FilterMenu("Keymaps", keymaps,
    (res, key) => {
      },
    (winid) => {
...
export def OpenReviewFile(line: string)
  var pr_id = line->split()[0]
  system($'gh pr checkout {pr_id}')
  git.PRreview()
...
export def PRlist()
  var pr_list = systemlist('gh pr list')
  popup.FilterMenu("PR list", pr_list,
    (res, key) => {
      var pr_id = res.text->split()[0]
      if key == "\<C-j>"
        exe $"split new"
        exe $":0r !gh pr view {pr_id}"
        exe $":r !gh pr view -c {pr_id}"
        exe "set ft=markdown"
      elseif key == "\<C-v>"
        exe $"vert split new"
        exe $":0r !gh pr view {pr_id}"
...
export def GitLog()
  var git_log = systemlist('git log --oneline')
  popup.FilterMenu("Git log", git_log,
    (res, key) => {
      var cmd = 'e new'
      if key == "\<C-j>"
        cmd = 'split new'
      ...
      exe cmd
      setlocal bufhidden=wipe buftype=nofile nobuflisted noswapfile ft=git
      exe $'r !git show {res.text->split(" ")[0]}'
      exe 'norm! ggdd'
    },
    (winid) => {
      win_execute(winid, "syn match FilterMenuDirectorySubtle '^.*[\\/]'")
...
export def GitBranch()
  var git_branch = systemlist('git branch -a -l')
  popup.FilterMenu("Git branch", git_branch,
    (res, key) => {
      if key == "\<CR>"
        system($'git checkout {res.text->substitute("* ", "", "")->split(" ")[0]}')
      endif
    },
    (winid) => {
      win_execute(winid, "syn match FilterMenuDirectorySubtle '^.*[\\/]'")
...
export def Marks()
  var marks = getmarklist()
    ...
    ->mapnew((_, v) => ({
      key: $"{v.mark}",
      text: $"{v.mark} " .. get(v, 'file', bufname()) ..  $" ({v.pos[1]})",
      linenr: v.pos[1],
