...
export def FilterMenu(title: string, items: list<any>, Callback: func(any, string), Setup: func(number) = null_function, close_on_bs: bool = false, live_grep: bool = false)
  ...
  var minwidth = &columns - 3
  # var pos_top = ((&lines - height) / 2) - 1
  var pos_top = &lines - height - 3
  ...
  def UpdatePopups(winid: number)
    popup_setoptions(winid, {title: $" ({items_count > 0 ? filtered_items[0]->len() : 0}/{items_count}) {title} {bordertitle[0]} {prompt} {bordertitle[1]}" })
    # popup_settext(winid, $"> {prompt}{popupcursor}")
